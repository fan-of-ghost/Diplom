...
public class ScheduleCertificatesController {
    @FXML
    ...
    private List<LocalDate> certificateDates;
    ...
    public void initialize() {
        ...
        loadCertificateDatesFromDatabase();
    ...
    private void loadCertificateDatesFromDatabase() {
        certificateDates = new ArrayList<>();
        try {
            DB db = DB.getBase();
            Connection conn = db.getDbConnection();
            Statement stmt = conn.createStatement();
            ResultSet rs = stmt.executeQuery("SELECT дата_использования FROM График_сертификатов");

            while (rs.next()) {
                certificateDates.add(LocalDate.parse(rs.getString("дата_использования")));
            }
    ...
    private void createContextMenu(boolean isHighlighted) {
        ...
        item1.setOnAction(event -> {
            ...
            try {
                WindowsActions.openWindow("Заезды на " + selectedDate, "racesCertificatesOnDay.fxml");
            } catch (IOException e) {
        ...
        for (int i = 0; i < 6; i++) {
            for (int j = 0; j < 7; j++) {
                ...
                if (i == 0 && j < dayOfWeekValue - 1) {
                    ...
                    boolean isHighlighted = certificateDates.contains(date);
        ...
        dayLabel.setOnMouseClicked(event -> {
            ...
            if (event.getButton().name().equals("SECONDARY")) {
                if (previouslySelectedLabel != null) {
                    ...
                    if (certificateDates.contains(LocalDate.of(year, month, Integer.parseInt(previouslySelectedLabel.getText())))) {
